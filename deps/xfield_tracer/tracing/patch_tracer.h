                       ...
                       std::map<EdgeVert,size_t> &EdgeDirVert)
{
    ...
    for (size_t i=0;i<TraceSet.size();i++)
    {
        ...
        for (size_t j=0;j<Limit;j++)
        {
            ...
            //                Selected[i]=true;
            //                MeshTraces(VFGraph,TraceSet,Selected,mesh);
            //                vcg::tri::io::ExporterPLY<MeshType>::Save(VFGraph.Mesh(),"double_direction_domain.ply",vcg::tri::io::Mask::IOM_FACECOLOR);
            //                vcg::tri::io::ExporterPLY<MeshType>::Save(mesh,"double_direction_error.ply",vcg::tri::io::Mask::IOM_FACECOLOR);
            //                assert(0);
            //            }
...
private:
    ...
    void SanitizePartitions(std::vector<size_t> &Partitions, bool IsLoop)
    {
        // prepare EdgeSet
        ...
        size_t limit = (IsLoop)?Partitions.size():Partitions.size()-2;

        for (size_t i=0; i<limit; i++){
            do {
                size_t IndexV0 = VertexFieldGraph<MeshType>::NodeVertI(Partitions[i]);
                size_t IndexV2 = VertexFieldGraph<MeshType>::NodeVertI(Partitions[(i+2)%Partitions.size()]);
                std::pair<size_t,size_t> key(std::min(IndexV0,IndexV2),
                                             std::max(IndexV0,IndexV2));
                if(EdgeSet.count(key) != 0){
                    Partitions.erase(Partitions.begin()+(i+1)%Partitions.size());
                    limit--;
                    //std::cerr << "SanitizePathNodes: removed a node" << std::endl;
                } else {
                    break;
                }
            }while (true);
                              ...
                              bool AllowLoopish)
    {
        ...
        for (size_t i=0;i<Candidates.size();i++)
        {
            ...
            SanitizePartitions(Candidates[i].PathNodes, Candidates[i].IsLoop);
            ...
            int AfterFirstIntersection;

            t0=clock();
            bool collide = CollideWithCandidateSet(VFGraph,Candidates[i], LastIntersection, AfterFirstIntersection, ChoosenPaths);
            t1=clock();
            time_collide+=t1-t0;
            ...
            if (collide)
            {
                if(AllowLoopish && Candidates[i].IsLoop && LastIntersection != -1 && AfterFirstIntersection != Candidates[i].PathNodes.size())
                {
                    //std::cerr << "Cut From " << LastIntersection << " To " << AfterFirstIntersection << "(Loop Length: " << Candidates[i].PathNodes.size() << ")" << std::endl;
                    
                    std::vector<size_t> sub_vec {Candidates[i].PathNodes.begin(), Candidates[i].PathNodes.begin()+LastIntersection+1};
                    Candidates[i].PathNodes.erase(Candidates[i].PathNodes.begin(), Candidates[i].PathNodes.begin()+AfterFirstIntersection);
                    Candidates[i].PathNodes.insert(Candidates[i].PathNodes.end(), sub_vec.begin(), sub_vec.end());
                    Candidates[i].IsLoop = false;
                    std::cerr << "New paths size: " << Candidates[i].PathNodes.size() << std::endl;
                }
                else{
                    To_Delete[i] = true;
                    continue;
                ...
                //std::cerr << "No collision, add loop." << std::endl;
            }
                                ...
                                bool AllowLoopish)
    {
        ...
        while (ChooseNextByDistance(UseVertNeeds,UsePartitionNeeds,
                                    ...
                                    time_update_dist,time_add, AllowLoopish))
        {}
        //        std::cout<<"done"<<std::endl;
        //        //            step=(step+1)%100;
        //        //            if (step!=0)continue;
...
private:
    ...
    bool RemoveIfPossible(size_t IndexPath)//,bool Do_Really_Remove=true)
    {
        ...
        check_quality_functor=true; //originally false
...
public:
                         ...
                         std::vector<size_t> &VertMap)
    {
        ...
        //        std::vector<bool> Selected(ChoosenPaths.size(),false);
        //        //Selected[i]=true;
        //        MeshTraces(VFGraph,ChoosenPaths,Selected,mesh);
        //        //vcg::tri::io::ExporterPLY<MeshType>::Save(VFGraph.Mesh(),"double_direction_domain.ply");
        //        vcg::tri::io::ExporterPLY<MeshType>::Save(mesh,"double_direction_error.ply",vcg::tri::io::Mask::IOM_FACECOLOR);
    ...
    //bool checkOnlylastConfl=false)
    {
        ...
        if (ChMode==Fartest)
            ChooseGreedyByDistance(UseNodeVal,UsePartitionNeeds,AllowTraceLoopish);
        else
        {
    ...
    void TraceBorderAndLoops(bool UsePartitionNeeds)
    {
        ...
        ChooseGreedyByDistance(false,UsePartitionNeeds,AllowTraceLoopish);
