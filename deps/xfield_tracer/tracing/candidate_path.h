...
bool CollideCandidates(VertexFieldGraph<MeshType> &VFGraph,
                       ...
                       const CandidateTrace &CT1)
{
    return (VertexFieldQuery<MeshType>::CollideTraces(VFGraph,CT0.PathNodes,CT0.IsLoop, CollisionPointsOnPaths, IntersectionPointsOnPaths,
                                                      CT1.PathNodes,CT1.IsLoop));
}
...
bool CollideWithCandidateSet(VertexFieldGraph<MeshType> &VFGraph,
                             const CandidateTrace &TestTrace,
                             int &LastIntersectionBeforeCollide,
                             int &FirstIntersectionAfterCollide,
                             const std::vector<CandidateTrace> &TraceSet)
{
    std::set<size_t> CollisionPointsOnPath;
    std::set<size_t> IntersectionPointsOnPath;

    LastIntersectionBeforeCollide = -1;
    FirstIntersectionAfterCollide = TestTrace.PathNodes.size();

    bool collide = false;
    
    for (size_t i=0;i<TraceSet.size();i++)
    {
        collide |= CollideCandidates<MeshType>(VFGraph,TestTrace, CollisionPointsOnPath, IntersectionPointsOnPath, TraceSet[i]);
    }

    if(collide){
        for (size_t i=0;i<TestTrace.PathNodes.size();i++){
            if(CollisionPointsOnPath.count(i) > 0){
                break;
            }else if(IntersectionPointsOnPath.count(i) > 0){
                LastIntersectionBeforeCollide = i;
            }
        }


        for (int i=TestTrace.PathNodes.size()-1;i>=0;i--){
            if(CollisionPointsOnPath.count(i) > 0){
                break;
            }else if(IntersectionPointsOnPath.count(i) > 0){
                FirstIntersectionAfterCollide = i;
            }
        }
    }
