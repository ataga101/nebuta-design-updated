...
public:
        ...
        for (size_t s=0;s<SameV.size();s++)
        {
            ...
            for (size_t i=Start0;i<Limit0;i++)
            {
                ...
                for (size_t j=Start1;j<Limit1;j++)
                {
                    ...
                    VIndex1.push_back(IndexV1);
                    VIndex1.push_back(IndexV2);
                    if (CollideSubSequence(VFGraph,VIndex0,VIndex1)){
                        UpdateSameVInfoWhenCollision(VFGraph, TraceN0, SameV, SameVInfo, i);
                        ret = true;
                    }
                }
            }
    ...
    static void UpdateSameVInfoWhenCollision(const VertexFieldGraph<MeshType> &VFGraph,
                                        const std::vector<size_t> &TraceN0, 
                                        const std::vector<size_t> &SameV,
                                        std::vector<bool> &IsSameVCollision,
                                        const size_t &CollisionIndex
                                        )
    {
        auto SameVIterator = std::find(SameV.begin(), SameV.end(), VertexFieldGraph<MeshType>::NodeVertI(TraceN0[CollisionIndex]));
        assert(SameVIterator != SameV.end());
        size_t SameVIndex = std::distance(SameV.begin(), SameVIterator);
        IsSameVCollision[SameVIndex] = true;
    }
    
    static void AddToCollisionAndIntersectionSets(const VertexFieldGraph<MeshType> &VFGraph,
                                        const std::vector<size_t> &TraceN0, 
                                        const std::vector<size_t> &SameV,
                                        const std::vector<bool> &SameVInfo,
                                        std::set<size_t> &CollisionPoints,
                                        std::set<size_t> &IntersectionPoints)
    {
        assert(SameV.size() == SameVInfo.size());
        for(int i=0;i<SameV.size();i++){
            size_t IndexForTrace;
            for(int j=0;j<TraceN0.size();j++){
                if(VertexFieldGraph<MeshType>::NodeVertI(TraceN0[j]) == SameV[i]){
                    IndexForTrace = j;
                    break;
                }
            }
            if(SameVInfo[i]){
                CollisionPoints.insert(IndexForTrace);
            }else{
                IntersectionPoints.insert(IndexForTrace);
            }
        }
    }
    ...
    static bool CollideTraces(VertexFieldGraph<MeshType> &VFGraph,
                              const std::vector<size_t > &TraceN0,
                              bool IsLoopTr0,
                              std::set<size_t> &CollisionPointsOnPaths,
                              std::set<size_t> &IntersectionPointsOnPaths,
                              const std::vector<size_t > &TraceN1,
                              bool IsLoopTr1)
    {
        assert(TraceN0.size()>1);
        ...
        for (size_t i=0;i<TraceN0.size();i++)
        {
            size_t IndexV=VertexFieldGraph<MeshType>::NodeVertI(TraceN0[i]);
            VertexType *v=&VFGraph.Mesh().vert[IndexV];
            if (vcg::tri::IsMarked(VFGraph.Mesh(),v)){
                SameV.push_back(IndexV);
                SameVInfo.push_back(false);
            }
        }
        if (SameV.size()==0){
            return false;
        ...
        for (size_t i=0;i<Limit0;i++)
        {
            ...
            std::pair<size_t,size_t> Key0(std::min(VIndex0,VIndex1),std::max(VIndex0,VIndex1));
            //check if already
            for (size_t j=0;j<Limit1;j++)
            {
                size_t VIndex0=VertexFieldGraph<MeshType>::NodeVertI(TraceN1[j]);
                size_t VIndex1=VertexFieldGraph<MeshType>::NodeVertI(TraceN1[(j+1)%TraceN1.size()]);
                std::pair<size_t,size_t> Key1(std::min(VIndex0,VIndex1),std::max(VIndex0,VIndex1));
                if (Key0==Key1){
                    UpdateSameVInfoWhenCollision(VFGraph, TraceN0, SameV, SameVInfo, i);
                    ret = true;
        ...
        //first check if there is the same node (or opposite in M2)
        VFGraph.UnMarkAll();
        if (IsLoopTr1)//in this case mark all
        {
            for (size_t i=0;i<TraceN1.size();i++)
            {
                VFGraph.Mark(TraceN1[i]);
                VFGraph.Mark(VertexFieldGraph<MeshType>::TangentNode(TraceN1[i]));
            }
        }
        else {
            //first and last without tangent
            VFGraph.Mark(TraceN1[0]);
            for (size_t i=1;i<TraceN1.size()-1;i++)
            {
                VFGraph.Mark(TraceN1[i]);
                VFGraph.Mark(VertexFieldGraph<MeshType>::TangentNode(TraceN1[i]));
            }
            VFGraph.Mark(VertexFieldGraph<MeshType>::TangentNode(TraceN1.back()));
        }

        for (size_t i=0;i<TraceN0.size();i++)
        {
            size_t NodeN1=TraceN0[i];
            if (VFGraph.IsMarked(NodeN1)){
                UpdateSameVInfoWhenCollision(VFGraph, TraceN0, SameV, SameVInfo, i);
                ret = true;
            }
        }

        ret |= CollideSubSequence(VFGraph,TraceN0,TraceN1,IsLoopTr0,IsLoopTr1, SameV, SameVInfo);
        if(ret){
            AddToCollisionAndIntersectionSets(VFGraph, TraceN0, SameV, SameVInfo, CollisionPointsOnPaths, IntersectionPointsOnPaths);
        }
        return ret;
    }
