...
void WriteUnsolvedStats(const std::vector<PatchType> &PatchTypes)
{
    ...
    for (size_t i=0;i<PatchTypes.size();i++)
    {
        ...
        case NoQualityMatch:numnoquality++; break;
        default: numOK++;
        }
    }
    ...
    std::cout<<"*No Quality Match:"<<numnoquality<<std::endl;
                      ...
                      bool DebugMsg=true)
{
    ...
    SolveSubPatches(PTr,onlyneeded,false, true, DebugMsg);
...
template <class MeshType, class PatchQualityFunctor>
void SaveCSV(PatchTracer<MeshType, PatchQualityFunctor> &PTr,
             const std::string &pathProject,
             const size_t CurrNum)
{
    typename PatchTracer<MeshType, PatchQualityFunctor>::PatchInfoType PInfo;
    PTr.GetInfo(PInfo);
...
template <class MeshType, class PatchQualityFunctor>
void SaveAllData(PatchTracer<MeshType, PatchQualityFunctor> &PTr,
                 const std::string &pathProject,
                 const size_t CurrNum,
                 bool subdivide_when_save,
                 bool save_origFace)
