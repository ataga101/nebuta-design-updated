//
// Created by Naoki Agata on 2023/04/10.
//
...
namespace tri_strip{
                             ...
                             double normal_dot_prod_threshold = 0.8)
    {
        ...
        for(int candidate_size=1; candidate_size<=bnd_loop.size()-2; candidate_size++){
            ...
            for(int start_vert_idx=0; start_vert_idx<bnd_loop.size(); start_vert_idx++){
                ...
                double valid_min_score = std::numeric_limits<double>::max();
                size_t valid_next_vert = -1;
                bool valid_next_vert_found = false;
                ...
                for(int candidate_idx=0; candidate_idx<candidate_size; candidate_idx++){
                    ...
                    Eigen::Vector3d e1 = V.row(bnd_loop[start_vert_idx]) - V.row(bnd_loop[target_idx]);
                    Eigen::Vector3d e2 = V.row(bnd_loop[end_vert_idx]) - V.row(bnd_loop[target_idx]);
                    Eigen::Vector3d n_now = e1.cross(e2).normalized();

                    bool now_is_normal_valid = true;
                    if (memo[start_vert_idx][target_idx].next_vert != -1) {
                        Eigen::Vector3d e1_prev1 =
                                V.row(bnd_loop[start_vert_idx]) - V.row(bnd_loop[memo[start_vert_idx][target_idx].next_vert]);
                        Eigen::Vector3d e2_prev1 =
                                V.row(bnd_loop[target_idx]) - V.row(bnd_loop[memo[start_vert_idx][target_idx].next_vert]);
                        Eigen::Vector3d n_prev_1 = e1_prev1.cross(e2_prev1).normalized();
                        now_is_normal_valid &= n_now.dot(n_prev_1) > normal_dot_prod_threshold && memo[start_vert_idx][target_idx].is_normal_valid;
                    }

                    if (memo[target_idx][end_vert_idx].next_vert != -1) {
                        Eigen::Vector3d e1_prev2 =
                                V.row(bnd_loop[target_idx]) - V.row(bnd_loop[memo[target_idx][end_vert_idx].next_vert]);
                        Eigen::Vector3d e2_prev2 =
                                V.row(bnd_loop[end_vert_idx]) - V.row(bnd_loop[memo[target_idx][end_vert_idx].next_vert]);
                        Eigen::Vector3d n_prev_2 = e1_prev2.cross(e2_prev2).normalized();
                        now_is_normal_valid &= n_now.dot(n_prev_2) > normal_dot_prod_threshold && memo[target_idx][end_vert_idx].is_normal_valid;
                    }
                    ...
                    if(now_score < valid_min_score && now_is_normal_valid){
                        valid_min_score = now_score;
                        valid_next_vert = target_idx;
                        valid_next_vert_found = true;
                    }
                }
                assert(next_vert != -1);
                if (valid_next_vert_found) {
                    memo[start_vert_idx][end_vert_idx].is_valid = true;
                    memo[start_vert_idx][end_vert_idx].cost = valid_min_score;
                    memo[start_vert_idx][end_vert_idx].next_vert = valid_next_vert;
                    memo[start_vert_idx][end_vert_idx].is_normal_valid = true;
                } else {
                    memo[start_vert_idx][end_vert_idx].is_valid = true;
                    memo[start_vert_idx][end_vert_idx].cost = min_score;
                    memo[start_vert_idx][end_vert_idx].next_vert = next_vert;
                    memo[start_vert_idx][end_vert_idx].is_normal_valid = false;
                }
            }
        }
    }
    ...
    void approximate_single_patch(Eigen::MatrixXd &V, Eigen::MatrixXi &F, Eigen::MatrixXd &resultV, Eigen::MatrixXi &resultF, triangle_strip_quality_mode mode){
        std::vector<Eigen::Index> bnd_loop;
        Eigen::VectorXi L;
