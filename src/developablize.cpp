...
namespace rp {
  ...
  void degenerate_internal_vertices(const Eigen::MatrixX3i &F, const Eigen::MatrixX3d &V,
                                    ...
                                    Eigen::VectorXi &V2NV,
                                    Eigen::VectorXi &NV2V) {
    Eigen::MatrixXi _F(F), _resultF(resultF);
    Eigen::MatrixXd _V(V), _resultV(resultV);

    igl::connect_boundary_to_infinity(V, F, _V, _F);

    igl::decimate_cost_and_placement_callback shortestEdgeAndMidpoint = [&](const int e,
    ...
    Eigen::VectorXi _;
    rp::decimate(_V, _F, shortestEdgeAndMidpoint, maxM, _resultV, _resultF, _, NV2V);
  ...
  void flip_edge(const int &edgeId,
    ...
    for (int i = 0; i < 2; i++) {
      for (int j = 0; j < 2; j++) {
        ...
        if (EF(e0, 0) == EF(e1, 0) || EF(e0, 0) == EF(e1, 1) || EF(e0, 1) == EF(e1, 0) || EF(e0, 1) == EF(e1, 1)) {
          nextEids = {};
          return;
        }
      }
    ...
    // if (edgeId == 1272) {
    //   std::cout << "ef: " << EF.row(edgeId) << std::endl;
    //   std::cout << "ev: " << EV.row(edgeId) << std::endl;
    //   std::cout << "f1: " << F.row(faces(0)) << std::endl;
    //   std::cout << "f2: " << F.row(faces(1)) << std::endl;
    //   std::cout << "fe1: " << FE.row(faces(0)) << std::endl;
    //   std::cout << "fe2: " << FE.row(faces(1)) << std::endl;
    //   std::cout << std::endl;
    // }
  ...
  bool needs_inversion(const Eigen::MatrixX3i &F, const Eigen::MatrixXi &EF,
    ...
    std::vector<std::vector<size_t>> otherEdgeIds = {{}, {}};
    ...
    if (otherVids.size() == 2) {
      ...
      if (pathIds(NV2V(otherVids[0])) != pathIds(NV2V(otherVids[1])) && pathIds(NV2V(EV(edgeId, 0))) == pathIds(NV2V(EV(edgeId, 1)))) {
        
        return true;
      } else if (pathIds(NV2V(EV(edgeId, 0))) != pathIds(NV2V(EV(edgeId, 1))) && pathIds(NV2V(otherVids[0])) == pathIds(NV2V(otherVids[1]))) {
      ...
      // diffがデカければ入れ替え
      if (useDiff) {
        double diffCross = std::abs(params(NV2V(otherVids[0])) - params(NV2V(otherVids[1])));
        double diff = std::abs(params(NV2V(EV(edgeId, 0))) - params(NV2V(EV(edgeId, 1))));
        if (diff > diffCross) {
          return true;
        }
      }
  ...
  void edge_inversion(/*const Eigen::VectorXd &zeroFormV,*/ const Eigen::VectorXi &NV2V,
    ...
    for (int i = 0; i < EV.rows(); i++) {
      if (needs_inversion(resultF, EF, FE, EV, NV2V, /*zeroFormV,*/ i, pathIds, useDiff) && !isConstrainedEdge(i)) {
  ...
  // void divide_border(Eigen::MatrixX3d &resultV, Eigen::MatrixX3i &resultF, const std::vector<std::vector<size_t>> &newPathVids) {
  //   rp::divide_borders::process(resultV, resultF, newPathVids);
  //   std::vector<std::vector<size_t>> newPaths = {}, VV = {};

  //   std::vector<std::vector<size_t>> 

  //   igl::adjacency_list(resultF, VV);
  //   for (int i = 0; i < newPathVids.size(); i++) {
  //     int firstVid = newPathVids[i][0], lastVid = newPathVids[i][newPathVids[i].size() - 1];
  //     int v0 = VV[firstVid][0], v1 = VV[lastVid][0];

  //     std::vector<int> p1 = {firstVid, v0}, p2 = {lastVid, v1};

  //     if (!isBoundary(firstVid)) {
  //       newPaths.emplace_back(p1);
  //     }
  //     if (!isBoundary(firstVid)) {
  //       newPaths.emplace_back(p2);
  //     }
  //   }

  //   rp::divide_borders::process(resultV, resultF, newPaths);
  // }
  ...
  void developablize(const Eigen::MatrixX3i &F, const Eigen::MatrixX3d &V,
    ...
    /*
    Eigen::VectorXd _zeroFormV(zeroFormV.rows());
    for (int i = 0; i < _zeroFormV.rows(); i++) {
      _zeroFormV(i) = zeroFormV(J(i));
    }
    */
    ...
    std::vector<std::vector<size_t>> newPathVids;
